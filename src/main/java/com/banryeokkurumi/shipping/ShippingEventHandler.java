...
class ShippingEventHandler {
    ...
    ShippingEventHandler(ShippingApplicationService service,ApplicationEventPublisher events,Clock clock,
                         @Value("${app.shipping.ship-delay:1m}")Duration shipDelay,@Value("${app.shipping.delivery-delay:2m}")Duration deliveryDelay){
        this.service=service;this.events=events;this.clock=clock;this.shipDelay=shipDelay;this.deliveryDelay=deliveryDelay;}
    @ApplicationModuleListener void on(CommerceEvents.ShipmentRequested event){service.create(event.orderId(),event.memberLoginId(),event.address());events.publishEvent(new CommerceEvents.ShipmentCreated(UUID.randomUUID(),now(),1,event.orderId()));}

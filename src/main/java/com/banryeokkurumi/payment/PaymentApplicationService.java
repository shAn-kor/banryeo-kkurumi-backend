...
public class PaymentApplicationService {
    private final PaymentRepository repository; private final Clock clock; private final Duration reconciliationDelay; private final Duration reconciliationWindow;
    public PaymentApplicationService(PaymentRepository repository,Clock clock,
                                     @Value("${app.payment.reconciliation-delay:10s}") Duration reconciliationDelay,
                                     @Value("${app.payment.reconciliation-window:10m}") Duration reconciliationWindow){this.repository=repository;this.clock=clock;this.reconciliationDelay=reconciliationDelay;this.reconciliationWindow=reconciliationWindow;}
